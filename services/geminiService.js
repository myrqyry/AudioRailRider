...
`;
const PROMPT_TEXT = (duration, bpm, energy, spectralCentroid, spectralFlux) => `
 ...
 */
const inlineAudioPartFromFile = async (file) => {
...
export const uploadFile = async (ai, file, mimeType) => {
    try {
        const payload = { file, config: { mimeType } };
        // Prefer the SDK method if available
        const raw = await ai.files.upload(payload);
        // Normalize common shapes
        const uri = raw?.uri || raw?.fileUri || raw?.id || raw?.name && `file://${raw.name}` || null;
        const normalized = {
            uri,
            mimeType: raw?.mimeType || raw?.contentType || mimeType || (file && file.type) || 'application/octet-stream',
            name: raw?.name || file.name,
            size: raw?.size || (file && file.size) || null,
            raw,
        };
        if (!normalized.uri) {
...
export const listFiles = async (ai, opts) => {
    try {
        const args = opts ? { ...opts } : {};
        const res = await ai.files.list(args);
...
export const getFileMetadata = async (ai, fileUri) => {
    try {
        const res = await ai.files.get({ uri: fileUri });
...
export const deleteFile = async (ai, fileUri) => {
    try {
        // Some SDKs expect { uri } others { fileUri } - try both shapes
        const res = await ai.files.delete?.({ uri: fileUri }) ?? await ai.files.delete?.({ fileUri }) ?? null;
...
export const downloadFile = async (ai, fileUri) => {
    try {
        // Attempt SDK download helper
        const res = await ai.files.download?.({ uri: fileUri }) ?? await ai.files.download?.({ fileUri }) ?? null;
        if (!res) {
        ...
        if (typeof res.data === 'string') {
            try {
                ...
                return { buffer: binary, size: binary.length ?? binary.byteLength ?? null, raw: res };
            }
            catch (e) {
                // Fallback: return raw
                return { raw: res };
 ...
 */
export const prepareAudioPart = async (ai, file, maxInlineBytes = MAX_INLINE_BYTES) => {
    // Normalize MIME: if file.type is present, strip any parameters (e.g., "audio/mpeg; charset=binary").
    // If file.type is empty, fall back to extension-based detection.
    const rawType = file.type && file.type.trim() !== '' ? file.type : detectMimeFromName(file.name);
    const detectedMime = typeof rawType === 'string' ? rawType.split(';')[0].trim() : rawType;
...
export const generateRideBlueprint = async (audioFile, duration, bpm, energy, spectralCentroid, spectralFlux) => {
...
export const transcribeAudioFile = async (file, timestamps) => {
    ...
    if (file instanceof File) {
        ...
        audioFile = new File([file], `audio.${ext}`, { type: mime });
    }
...
export const countAudioTokens = async (file) => {
    ...
    try {
        ...
        try {
            const sizeInfo = typeof audioFile.size === 'number' ? `${audioFile.size} bytes` : 'unknown size';
            const mode = audioPart.inlineData ? 'inline' : audioPart.fileData ? 'upload' : 'unknown';
        ...
        else if (response && typeof response.total_tokens === 'number') {
            totalTokens = response.total_tokens;
        }
        else if (response && typeof response.tokens === 'number') {
 ...
 */
export const generateStructuredData = async (prompt) => {
