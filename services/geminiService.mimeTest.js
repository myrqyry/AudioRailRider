...
class MockAIClient extends GoogleGenAI {
    files = {
        async upload({ file, config }) {
            await new Promise((r) => setTimeout(r, 10));
            return { uri: `file://uploaded/${file.name}`, mimeType: config?.mimeType ?? file.type ?? 'application/octet-stream' };
...
async function runTestCase(testName, file) {
    try {
        const part = await prepareAudioPart(mockAi, file);
        const mode = 'inlineData' in part ? 'inline' : 'fileData' in part ? 'upload' : 'unknown';
        console.log(`[PASS] ${testName} => accepted. Mode: ${mode}. Detected mime (returned):`, part.fileData?.mimeType ?? part.inlineData?.mimeType);
...
(async () => {
    ...
    for (const mime of SUPPORTED_MIMES) {
        ...
        const file = { name: `song.${ext}`, type: mime, size: LARGE_FILE_SIZE };
    ...
    for (const [ext, mime] of Object.entries(SUPPORTED_EXT_TO_MIME)) {
        const file = { name: `instrumental.${ext}`, type: '', size: LARGE_FILE_SIZE };
    ...
    for (const ext of Object.keys(SUPPORTED_EXT_TO_MIME)) {
        const file = { name: `weird.${ext}`, type: 'text/plain', size: LARGE_FILE_SIZE };
    ...
    const txtFile = { name: 'notes.txt', type: '', size: LARGE_FILE_SIZE };
    ...
    const pdfFake = { name: 'notaudio.mp3', type: 'application/pdf', size: LARGE_FILE_SIZE };
    ...
    const positiveTests = summary.filter(s => s.name.startsWith('supported-mime') || s.name.startsWith('ext-fallback'));
