...
export class VisualEffects {
    ...
    constructor(scene: THREE.Scene, trackData: TrackData) {
        ...
        const trackMesh = new THREE.Mesh(tubeGeometry, this.trackGlowMaterial);
    ...
    update(elapsedTime: number, audioFeatures: any, cameraPosition: THREE.Vector3) {
        ...
        if (audioFeatures && audioFeatures.loudness) {
            const midLoudness = audioFeatures.loudness.specific.slice(7, 14).reduce((a: number, b: number) => a + b, 0);
            const bassLoudness = audioFeatures.loudness.specific.slice(0, 5).reduce((a: number, b: number) => a + b, 0);
